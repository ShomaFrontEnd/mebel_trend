...
const Card = ({ imgPath, imgName, id }) => {
  ...
  return (
    ...
    <div className=" max-w-xs bg-white border hover:scale-105 border-gray-200 rounded-lg shadow duration-200 hover:shadow-xl">
      ...
      <Link className='relative' to={`/catalog/${imgName}/${id}`}>
      ...
      <div className="p-5">
        ...
        <Link to={`/catalog/${imgName}/${id}`} className="w-full justify-center inline-flex items-center px-3 py-2 text-sm font-bold text-center text-[#49788b] border border-[#518297] duration-300 rounded-lg hover:bg-[#518297] hover:text-white active:ring-4 active:outline-none active:ring-[#518297]">
          Подробнее
